...
export function StrengthPrediction({ onStepChange }: StrengthPredictionProps) {
  ...
  const handleOptimize = async () => {
    ...
    setResults({
      strength: {
        '1d': { value: 15.50, confidence: 0.5 },
        '3d': { value: 31.40, confidence: 1.0 },
        '28d': { value: 58.70, confidence: 1.5 }
      },
      chemical: {
        'SiO₂': 22.120,
        'Al₂O₃': 5.080,
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="space-x-4">
              <Button 
                ...
                size="lg"
                className={isOptimizing ? 'bg-gray-600' : results ? 'bg-green-600' : ''}
              >
                {isOptimizing ? '强度预测中...' : results ? '预测完成' : '开始预测'}
              </Button>
              {isOptimizing && (
                <Button 
                  variant="outline" 
