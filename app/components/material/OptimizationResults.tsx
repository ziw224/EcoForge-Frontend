...
export function OptimizationResults({ results }: OptimizationResultsProps) {
  ...
  const [previousResult, setPreviousResult] = useState<OptimizationResult | null>(null)
  const [useLogScale, setUseLogScale] = useState(false)
  const [comparisonGroup, setComparisonGroup] = useState('ratios')
  const [amplifyDifferences, setAmplifyDifferences] = useState(false)

  const handleTabChange = (value: string) => {
    const newResult = results.find(r => r.id.toString() === value) || results[0]
  ...
  const compareValues = (current: number, previous: number | undefined) => {
    ...
    if (diff > 0) return <ArrowUpIcon className="inline text-green-400 w-4 h-4 ml-1" />
    if (diff < 0) return <ArrowDownIcon className="inline text-red-400 w-4 h-4 ml-1" />
  ...
  const chemicalDifference = useMemo(() => {
    ...
    Object.keys(selectedResult.chemical).forEach(key => {
      diff[key] = selectedResult.chemical[key] - (previousResult.chemical[key] || 0)
    })
    return diff
  ...
  const prepareComparisonData = () => {
    ...
    const data = groups[comparisonGroup].map(key => {
      const values = results.map(result => 
        result[key] || result.strength[key] || result.chemical[key] || 0
      )
      const normalizedValues = amplifyDifferences ? normalizeData(values) : values
      return {
        name: key,
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
          <Tabs defaultValue={selectedResult.id.toString()} onValueChange={handleTabChange}>
            <TabsList className="grid w-full grid-cols-3">
              {results.map((result) => (
                <TabsTrigger key={result.id} value={result.id.toString()}>方案 {result.id}</TabsTrigger>
              ))}
            </TabsList>
            {results.map((result) => (
              <TabsContent key={result.id} value={result.id.toString()}>
                  ...
                  {['KH', 'N', 'P'].map((key) => (
                    <div key={key} className="text-center p-4 bg-gray-700 rounded-lg">
                      <div className="text-2xl font-bold text-white">
                        ...
                        {compareValues(result[key], previousResult?.[key])}
                      </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  {['1d', '3d', '28d'].map((day) => (
                    <div key={day} className="text-center p-4 bg-gray-700 rounded-lg">
                      <div className="text-2xl font-bold text-white">
                        {result.strength[day].toFixed(1)} MPa
                        {compareValues(result.strength[day], previousResult?.strength[day])}
                      </div>
                      <div className="text-sm text-gray-300">{day.replace('d', '天')}强度</div>
                    </div>
                  ))}
                </div>
                <Card className="mt-6">
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                      <LineChart data={prepareStrengthData(result.strength)}>
                        ...
                        <Tooltip />
                        <Legend />
                        <Line type="monotone" dataKey="强度" stroke="#8884d8" activeDot={{ r: 8 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                <Card className="mt-6">
                  ...
                  <CardContent>
                    <p className="mb-4 text-sm text-gray-600">
                      该方案的主要特点是 {result.KH > 0.92 ? '较高' : '较低'} 的 KH 值（{result.KH.toFixed(2)}），
                      {result.N > 2.35 ? '较高' : '较低'} 的硅率（{result.N.toFixed(2)}），
                      以及 {result.P > 1.45 ? '较高' : '较低'} 的铝率（{result.P.toFixed(2)}）。
                      这可能导致 {result.strength['28d'] > 58 ? '较高' : '较低'} 的28天强度（{result.strength['28d'].toFixed(1)} MPa）。
                    </p>
                    <div className="flex justify-end items-center space-x-2 mb-4">
                      ...
                      />
                      <Label htmlFor={`log-scale-${result.id}`}>使用对数刻度</Label>
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <ResponsiveContainer width="100%" height={400}>
                          ...
                          >
                            ...
                            <XAxis 
                              ...
                              tickFormatter={(value) => useLogScale ? `10^${value.toFixed(0)}` : value.toFixed(1)}
                            />
                            <YAxis dataKey="name" type="category" width={60} />
                            <Tooltip 
                              formatter={(value, name, props) => {
                                const originalValue = result.chemical[props.payload.name]
                                return [`${originalValue.toFixed(3)}%`, props.payload.name]
                              }}
                            />
                            <Legend />
                      ...
                      <div>
                        <Table>
                          ...
                          <TableBody>
                            {Object.entries(result.chemical).map(([name, value]) => (
                              <TableRow key={name}>
                                <TableCell>{name}</TableCell>
                                <TableCell className="text-right">{value.toFixed(3)}%</TableCell>
                                <TableCell className="text-right">
                                  {chemicalDifference[name] ? (
                                    <span className={chemicalDifference[name] > 0 ? 'text-green-500' : 'text-red-500'}>
            ...
            <div>
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={prepareComparisonData()} layout="vertical">
                  ...
                  <Tooltip 
                    formatter={(value, name, props) => {
                      const originalValue = props.payload[`原始值${name.slice(-1)}`]
                      return [`${originalValue.toFixed(3)}`, name]
                    }}
                  />
                  <Legend />
                  {results.map((_, index) => (
                    <Bar key={index} dataKey={`方案${index + 1}`} fill={COLORS[index]} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            </div>
