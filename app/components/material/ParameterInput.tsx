...
export function ParameterInput({
  ...
  const toggleParameterVisibility = (category: string, id: string) => {
    setParameters((prev) => ({
      ...prev,
      [category]: prev[category].map((param) =>
    ...
    >
      {editMode[category] && (
        <Switch
          ...
          onCheckedChange={() => toggleParameterVisibility(category, param.id)}
      ...
      {param.visible && (
        <Input
          ...
          className="max-w-[120px] bg-gray-700 text-white border-gray-600"
          placeholder="参数输入"
        />
      )}
    </div>
